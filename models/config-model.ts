...
export interface IConfig extends Document {
  ...
  valorCartaoAmarelo: number; // New
  valorCartaoVermelho: number; // New
...
const ConfigSchema: Schema<IConfig> = new Schema(
  {
    ...
    valorCartaoAmarelo: { type: Number, default: 5.0 }, // New
    valorCartaoVermelho: { type: Number, default: 10.0 }, // New
...
export async function getAppConfig(): Promise<IConfig> {
  ...
  if (!config) {
    ...
    config = await Config.create({
      ...
      valorCartaoAmarelo: 5.0, // New
      valorCartaoVermelho: 10.0, // New
