          ...
          <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
            ...
            <SheetContent side="right">
              <div className="flex flex-col gap-4 p-4">
                <button onClick={() => handleNavigation("/dashboard")} className="flex items-center gap-2 mb-4">
                ...
                {filteredLinks.map((link) => (
                  <button
                    key={link.href}
                    onClick={() => handleNavigation(link.href)}
                    className={`block py-2 text-lg transition-colors hover:text-primary ${pathname === link.href ? "text-primary font-semibold" : "text-muted-foreground"
                      }`}
                  >
                    {link.label}
                  </button>
